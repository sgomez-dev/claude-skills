# Upstream provenance — ads-server-side-tracking

This directory is a **vendored copy** of a third-party Agent Skill. It is not
authored here.

- Source repo: <https://github.com/AgriciDaniel/claude-ads>
- Tracked ref: `main`
- Upstream path: `skills/ads-server-side-tracking`
- Vendored commit: `ac21644933910419529bcf81efb95a9ca71edf81`
- Commit date: 2026-09-11T00:11:05+03:00
- Synced: 2026-09-16T10:07:52Z
- Scope: public

## Do not edit by hand

Local changes are overwritten on the next sync. To pull upstream changes:

```bash
./scripts/sync-external.sh ads-server-side-tracking
```

If you need behaviour that differs from upstream, either open a PR upstream or
fork the repo and point the manifest at your fork.

License: see `LICENSE` in this directory if present (the upstream project's
terms apply to this copy).
